...
class result_activity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val mangroveOrNot = inputM.bufferedReader().use { it.readLines() }
        ...
        if(photoBitmap!=null){
            ...
            handler.post {
                ...
                // if predict result is leaf
                if (leafNot == "Leaf"){
                    // predict NOT mangrove OR mangrove species
                    val predictionResult = predictMangroveSpecies(photoBitmap)

                    // extract classes name from file according to the index returned
                    resultTextView.text = mangroveOrNot[predictionResult].toString()
                    val words = mangroveOrNot[predictionResult].toString()
                    loading.isDismiss()

                    if (words == "NotMangrove"){
                        Toast.makeText(this, "This is not a mangrove leaf, please try again.", Toast.LENGTH_SHORT).show()
                        val CameraIntent = Intent(this, CustomCameraActivity::class.java)
                    ...
                    loading.isDismiss()
                    val start_intent = Intent(this, CustomCameraActivity::class.java)
    ...
    // predict not mangrove or species
    private fun predictMangroveSpecies(photoBitmap: Bitmap): Int {
        var tensorImage =  TensorImage(DataType.FLOAT32)
        // pass bitmap
        tensorImage.load(photoBitmap)
        ...
        val model = NMangroveOrNot.newInstance(this)
